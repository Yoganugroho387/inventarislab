...
class Borrowings extends CI_Controller {
    ...
    public function index() {
        ...
        $data = [
            ...
            'pageTitle' => 'Riwayat Transaksi Peminjaman Anda',
            'pageSubtitle' => 'Pantau status permohonan persetujuan peminjaman barang yang Anda ajukan.',
    ...
    public function create() {
        ...
        $data = [
            ...
            'pageTitle' => 'Pengajuan Peminjaman Baru',
            'pageSubtitle' => 'Buat draf formulir peminjaman untuk diajukan persetujuan ke kepala lab/admin.',
            'items' => $items,
            'prefix' => '/staff'
        ];
    ...
    public function store() {
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            redirect('staff/borrowings/create');
        } else {
            ...
            try {
                ...
                $this->activity_log_model->log($this->auth_lib->id(), 'Input Peminjaman', 'Mengajukan permohonan peminjaman untuk: ' . $borrowing_data['borrower_name']);

                // WhatsApp Notification
                if (!empty($borrowing_data['borrower_phone'])) {
                    $this->load->helper('whatsapp');
                    $msg = "Halo *" . $borrowing_data['borrower_name'] . "*,\n\nPengajuan peminjaman Anda dengan ID *#" . $borrowing_id . "* telah berhasil dibuat.\n\nStatus saat ini: *MENUNGGU PERSETUJUAN*.\nKami akan mengirimkan notifikasi WhatsApp setelah disetujui oleh admin.\n\nTerima kasih!";
                    send_whatsapp($borrowing_data['borrower_phone'], $msg);
                }

                $this->session->set_flashdata('success', 'Pengajuan peminjaman berhasil dibuat dan menunggu persetujuan Admin.');
                redirect('staff/borrowings');
            } catch (Exception $e) {
    ...
    public function view($id) {
        ...
        if (!$borrowing || $borrowing['created_by'] != $this->auth_lib->id()) {
    ...
    public function calendar_events() {
        ...
        $this->db->where('b.created_by', $this->auth_lib->id());
        ...
        foreach ($borrowings as $b) {
            ...
            $events[] = [
                ...
                'url' => url('/staff/borrowings/view/' . $b['id']),
