...
<div class="max-w-3xl bg-white p-6 rounded border border-slate-200 shadow-sm">
    <div class="border-b border-slate-100 pb-4 mb-5 flex items-center justify-between">
        <h2 class="text-xs font-semibold text-slate-700 uppercase tracking-wider">Form Edit Data Barang</h2>
    ...
    <form action="<?= url('/admin/items/update/' . $item['id']) ?>" method="POST" enctype="multipart/form-data" autocomplete="off">
        <?= csrf_field() ?>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <select name="item_type" id="item_type" required onchange="toggleTypeFields()"
                        class="mt-1 block w-full px-3 py-1.5 text-xs border border-slate-300 rounded focus:outline-none focus:ring-1 focus:ring-teal-500 focus:border-teal-500 bg-slate-50 focus:bg-white text-slate-800 transition duration-150">
                    <option value="inventaris" <?= old('item_type', $item['item_type']) === 'inventaris' ? 'selected' : '' ?>>Aset Inventaris Tetap (Dapat Dipinjam)</option>
                    <option value="habis_pakai" <?= old('item_type', $item['item_type']) === 'habis_pakai' ? 'selected' : '' ?>>Bahan Habis Pakai / Stok Komponen</option>
                </select>
            ...
            <div>
                ...
                <select name="category_id" id="category_id" required
                    ...
                    <?php foreach ($categories as $c): ?>
                        <option value="<?= $c['id'] ?>" <?= old('category_id', $item['category_id']) == $c['id'] ? 'selected' : '' ?>><?= esc($c['name']) ?></option>
                    <?php endforeach; ?>
                </select>
            ...
            <div>
                ...
                <select name="location_id" id="location_id" required
                    ...
                    <?php foreach ($locations as $l): ?>
                        <option value="<?= $l['id'] ?>" <?= old('location_id', $item['location_id']) == $l['id'] ? 'selected' : '' ?>><?= esc($l['name']) ?></option>
                    <?php endforeach; ?>
                </select>
            ...
            <div>
                ...
                <select name="unit_id" id="unit_id" required
                    ...
                    <?php foreach ($units as $u): ?>
                        <option value="<?= $u['id'] ?>" <?= old('unit_id', $item['unit_id']) == $u['id'] ? 'selected' : '' ?>><?= esc($u['name']) ?></option>
                    <?php endforeach; ?>
                </select>
            ...
            <!-- 7. Jumlah Stok -->
            <div>
                <label for="stock" class="block text-xs font-semibold text-slate-700">Jumlah Stok</label>
                <input type="number" name="stock" id="stock" required min="0"
                       value="<?= esc(old('stock', $item['stock'])) ?>"
                       class="mt-1 block w-full px-3 py-1.5 text-xs border border-slate-300 rounded focus:outline-none focus:ring-1 focus:ring-teal-500 focus:border-teal-500 bg-slate-50 focus:bg-white text-slate-800 transition duration-150">
                <?php if (error('stock')): ?>
            ...
            <div id="min_stock_field">
                ...
                <input type="number" name="minimum_stock" id="minimum_stock" min="0"
                       value="<?= esc(old('minimum_stock', $item['minimum_stock'])) ?>"
                       class="mt-1 block w-full px-3 py-1.5 text-xs border border-slate-300 rounded focus:outline-none focus:ring-1 focus:ring-teal-500 focus:border-teal-500 bg-slate-50 focus:bg-white text-slate-800 transition duration-150">
            ...
            <!-- 9. Kondisi Aset (Only showing for Inventaris) -->
            <div id="condition_field">
                <label for="condition_status" class="block text-xs font-semibold text-slate-700">Kondisi Aset</label>
                <select name="condition_status" id="condition_status"
                        class="mt-1 block w-full px-3 py-1.5 text-xs border border-slate-300 rounded focus:outline-none focus:ring-1 focus:ring-teal-500 focus:border-teal-500 bg-slate-50 focus:bg-white text-slate-800 transition duration-150">
                    <option value="tersedia" <?= old('condition_status', $item['condition_status']) === 'tersedia' ? 'selected' : '' ?>>Baik (Tersedia)</option>
                    <option value="rusak" <?= old('condition_status', $item['condition_status']) === 'rusak' ? 'selected' : '' ?>>Rusak Ringan/Berat</option>
                    <option value="maintenance" <?= old('condition_status', $item['condition_status']) === 'maintenance' ? 'selected' : '' ?>>Sedang Diservis (Perawatan)</option>
                </select>
            ...
            <div>
                ...
                <input type="number" name="procurement_year" id="procurement_year" min="1990" max="2100"
                       value="<?= esc(old('procurement_year', $item['procurement_year'])) ?>"
                       class="mt-1 block w-full px-3 py-1.5 text-xs border border-slate-300 rounded focus:outline-none focus:ring-1 focus:ring-teal-500 focus:border-teal-500 bg-slate-50 focus:bg-white text-slate-800 transition duration-150">
            </div>
            ...
            <div>
                ...
                <input type="text" name="funding_source" id="funding_source"
                       value="<?= esc(old('funding_source', $item['funding_source'])) ?>"
                       class="mt-1 block w-full px-3 py-1.5 text-xs border border-slate-300 rounded focus:outline-none focus:ring-1 focus:ring-teal-500 focus:border-teal-500 bg-slate-50 focus:bg-white text-slate-800 transition duration-150"
            ...
            <div>
                ...
                <input type="number" name="acquisition_price" id="acquisition_price" min="0" step="0.01"
                       value="<?= esc(old('acquisition_price', $item['acquisition_price'])) ?>"
                       class="mt-1 block w-full px-3 py-1.5 text-xs border border-slate-300 rounded focus:outline-none focus:ring-1 focus:ring-teal-500 focus:border-teal-500 bg-slate-50 focus:bg-white text-slate-800 transition duration-150">
            </div>

            <!-- 13. Upload Foto Barang & Preview -->
            <div>
                <label class="block text-xs font-semibold text-slate-700">Foto Barang (Format JPG/PNG, Maks 2MB)</label>
                <div class="mt-1 flex items-center space-x-4">
                    <!-- Thumbnail preview -->
                    <?php if (!empty($item['image']) && file_exists('./uploads/' . $item['image'])): ?>
                        <div class="relative w-14 h-14 border border-slate-200 rounded overflow-hidden shadow-sm flex-shrink-0 bg-slate-100">
                            <img src="<?= base_url('uploads/' . $item['image']) ?>" alt="preview" class="w-full h-full object-cover">
                        </div>
                    <?php else: ?>
                        <div class="w-14 h-14 rounded bg-slate-100 text-slate-400 flex items-center justify-center flex-shrink-0 border border-slate-200 shadow-sm">
                            <i data-lucide="image" class="w-5 h-5"></i>
                        </div>
                    <?php endif; ?>
                    
                    <input type="file" name="image" id="image" accept="image/*"
                           class="block w-full px-2 py-1 text-xs border border-slate-300 rounded bg-slate-50 text-slate-800 focus:outline-none focus:ring-1 focus:ring-teal-500 transition duration-150 cursor-pointer">
                </div>
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="description" id="description" rows="3"
                          class="mt-1 block w-full px-3 py-1.5 text-xs border border-slate-300 rounded focus:outline-none focus:ring-1 focus:ring-teal-500 focus:border-teal-500 bg-slate-50 focus:bg-white text-slate-800 transition duration-150"
                          placeholder="Masukkan spesifikasi rinci, seri barang, atau kelengkapan..."><?= esc(old('description', $item['description'])) ?></textarea>
            </div>
        </div>
